...
from graph_offline_imitation.networks.base          import ModuleContainer
from graph_offline_imitation.utils                  import utils
from graph_offline_imitation.envs.base              import Empty
...
class OffPolicyAlgorithm(Algorithm):
    ...
    def setup_datasets(self, env: gym.Env, total_steps: int):
        ...
        if env is None or isinstance(env, Empty) or self.offline_steps < 0:
    ...
    #         metrics["reward"]    = -np.inf  # purposefully set nan so we don't rewrite csv log
    #         metrics["length"]    = np.inf  # purposefully set nan so we don't rewrite csv log
    ...
    #         # Reset the environment
    #         self._current_obs    = self.env.reset()
    #         self.dataset.add(self._current_obs)  # Add the first time-step
    #         self._episode_length = 0
    #         self._episode_reward = 0
    #     else:
    #         self._current_obs    = next_obs
