...
class GoalConditionedAntDataset(HindsightReplayBuffer):
    ...
    def _data_generator(self):
        ...
        assert worker_info is None, "Kitchen Dataset does not support shared loading."
        ...
        for i in range(dataset["rewards"].shape[0]):
            obs                     = dataset["observations"][i, 2:].astype(np.float32)
            ...
            desired_goal[:2]        = dataset["infos/goal"][i].astype(np.float32)
            ...
            if done:
                if "next_observations" in dataset:
                    ...
                    temp_g[:2] = dataset["infos/goal"][i].astype(np.float32)
                    g_.append(temp_g)
                else:
