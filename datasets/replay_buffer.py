...
class ReplayBuffer(torch.utils.data.IterableDataset):
    ...
    def _alloc(self):
        ...
        if self.capacity is not None:
            ...
            # Next, write in the allocated data lazily using the generator until we are full.
...
class HindsightReplayBuffer(ReplayBuffer):
    ...
    def sample(self, batch_size: Optional[int] = None, stack: int = 1, pad: int = 0):
        ...
        if "horizon" in kwargs:
            ...
            horizon = -100 * np.ones_like(idxs, dtype=np.int64)
        ...
        # Compute the horizon
        if self.nstep > 1:
            horizon[her_idxs] = np.ceil((goal_idxs - obs_idxs[her_idxs]) / self.nstep).astype(np.int64)
        else:
        ...
        if self.strategy.startswith("any"):
            ...
            # The percentage sampled marginally is indicated by the fraction. ie any_0.5 will sample
    ...
    def _data_generator(self):
        ...
        for ep_filename in ep_filenames:
            ...
            if self.terminal_threshold is not None and obs[self.goal_key].dtype == np.float32:
                ...
                horizon         = -100 * np.ones(terminals.shape, dtype=np.int64)
