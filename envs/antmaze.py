...
class OfflineGoalAntWrapper(gym.ObservationWrapper):
  def __init__(self, env: Env):

    env.observation_space = gym.spaces.Dict(
        {
            "achieved_goal": gym.spaces.Box(low=np.full((29,), -np.inf), high=np.full((29,), np.inf), dtype=np.float32),
            # "observation":   gym.spaces.Box(low=np.full((27,), -np.inf), high=np.full((27,), np.inf), dtype=np.float32),
            "desired_goal":  gym.spaces.Box(low=np.full((29,), -np.inf), high=np.full((29,), np.inf), dtype=np.float32),
        }
    )
    super(OfflineGoalAntWrapper, self).__init__(env)
...
def make_offline_ant(env_name):
  ...
  if env_name in ['offline_ant_umaze', 'offline_goal_ant_umaze']:
    env = gym.make('antmaze-umaze-v2')
  elif env_name in ['offline_ant_umaze_diverse', 'offline_goal_ant_umaze_diverse']:
    env = gym.make('antmaze-umaze-diverse-v2')
  elif env_name in ['offline_ant_medium_play', 'offline_goal_ant_medium_play']:
    env = gym.make('antmaze-medium-play-v2')
  elif env_name in ['offline_ant_medium_diverse', 'offline_goal_ant_medium_diverse']:
    env = gym.make('antmaze-medium-diverse-v2')
  elif env_name in ['offline_ant_large_play', 'offline_goal_ant_large_play']:
    env = gym.make('antmaze-large-play-v2')
  elif env_name in ['offline_ant_large_diverse', 'offline_goal_ant_large_diverse']:
    env = gym.make('antmaze-large-diverse-v2')
  else:
    raise NotImplementedError
  
  if 'goal' in env_name:
    return OfflineGoalAntWrapper(env.env)
  else:
