...
class KitchenKettleMicrowaveBottomBurnerHingeV0(KitchenBase):
    TASK_ELEMENTS = ["kettle", "microwave", "bottom burner", "hinge cabinet"]
...
class KitchenDropGoalWrapper(gym.Wrapper):
    """
        Kitchen wrapper that only observe achieved goal (d = 30) instead of achieved goal + desired goal (d = 60).
    """
    def __init__(self, env: Env):
        super().__init__(env)
        obs_low, obs_high      = self.observation_space.low[:30], self.observation_space.high[:30]
...
class KitchenGoalConditionedWrapper(gym.Wrapper):
    ...
    def __init__(self, *args, path=None, test_fraction=0.05, **kwargs) -> None:
        ...
        self.end_points = masks.sum(axis=0).astype(np.int64)
