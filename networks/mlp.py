...
from graph_offline_imitation.networks.common import MLP, EnsembleMLP, LinearEnsemble
...
class DiscreteMLPDistance(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.ensemble_size = ensemble_size
        if self.ensemble_size > 1:
            self.mlp = EnsembleMLP(observation_space.shape[0], self.bins, ensemble_size=ensemble_size, **kwargs)
        else:
            self.mlp = MLP(observation_space.shape[0], self.bins, **kwargs)
...
class MLPDiscriminator(nn.Module):
    ...
    def forward(self, obs: torch.Tensor) -> torch.Tensor:
        return self.mlp(obs).squeeze(-1)  # Remove training 1
