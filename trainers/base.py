...
class OnlineTrainer(OfflineTrainer):
    ...
    def train(self, path: str):
        ...
        if os.path.exists(os.path.join(path, "final_model.pt")):
            ...
            metadata = self.model.load(os.path.join(path, "final_model.pt"), strict=True)
            current_step, steps, epochs = metadata["current_step"], metadata["steps"], metadata["epochs"]
            # Try to load the x axis value if we need to.
        else:
            current_step, steps, epochs = 0, 0, 0
        ...
        # obtain dataset and update env_step func
        if not hasattr(self.model, "dataset"):
            # self.model.setup_train_dataset()
            self.model.setup_datasets(self.model.env, self.total_steps)
        ...
        while current_step <= self.total_steps:
            # Online interaction with the env
            env_step(current_step, self.total_steps, timeit=profile)
            # Sample batch from the replay buffer
            batch = self.model.dataset.sample(batch_size=self.model.dataset.batch_size, stack=1, pad=0)
            # preprocess batch
            batch = format_batch(batch, timeit=profile)
            # run the train step
            train_step(batch, current_step, self.total_steps, timeit=profile)
            ...
            # epochs += 1 # no epoch for online learning