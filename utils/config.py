...
class Config(BareConfig):
    ...
    def get_model(self, device: Union[str, torch.device] = "auto"):
        assert self.parsed
        # Return the model
        alg_class                = vars(algs)[self["alg"]]
        dataset_class            = None if self["dataset"] is None else vars(datasets)[self["dataset"]]
        validation_dataset_class = (
            None if self["validation_dataset"] is None else vars(datasets)[self["validation_dataset"]]
        )
        network_class            = None if self["network"] is None else vars(networks)[self["network"]]
        optim_class             = None if self["optim"] is None else vars(torch.optim)[self["optim"]]
        processor_class         = None if self["processor"] is None else vars(processors)[self["processor"]]
...
class OnlineConfig(Config):
    ...
    def get_model(self, device: Union[str, torch.device] = "auto"):
        ...
        # May preload the offline dataset for online learning
        dataset_class            = None if self["dataset"] is None else vars(datasets)[self["dataset"]]
        validation_dataset_class = (
            None if self["validation_dataset"] is None else vars(datasets)[self["validation_dataset"]]
        )
        assert validation_dataset_class is None, "No validation dataset for Online Learning"
